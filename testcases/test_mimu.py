...
class TestMain:
    ...
    @pytest.mark.parametrize('args', yaml_case_data(read_yaml_key('config/config.yaml', 'case_data_path')))
    ...
    @pytest.mark.parametrize('args', yaml_case_data(read_yaml_key('config/config.yaml', 'case_data_path_post')))
    def test_post_api(self, args):
        ...
        response_json = response.text
        self.logger.info(
            '模块名称: {},测试目的: {} ,响应结果: {}'.format(args['story'], args['case_name'], response_json))
        assert response.status_code == 200
        return response_json
